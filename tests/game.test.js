...
test('dense contacts remove closing speed but preserve frictionless sliding',()=>{const point=(x,px,vy)=>({x,y:0,z:0,px,py:-vy,pz:0}),w=new RopeWorld(),a=point(0,-.02,.03),b=point(RADIUS,RADIUS+.02,-.01);w.ropes=[{nodes:[a],fade:0,attached:false,landed:true},{nodes:[b],fade:0,attached:false,landed:true}];w.collide();assert.ok(Math.abs((a.x-a.px)-(b.x-b.px))<1e-12);assert.ok(Math.abs((a.y-a.py)-.03)<1e-12);assert.ok(Math.abs((b.y-b.py)+.01)<1e-12);});
test('floor keeps nonzero friction while wire contacts stay frictionless',()=>{assert.ok(PHYSICS_PARAMETERS.floorFriction>0);assert.ok(PHYSICS_PARAMETERS.floorFriction<.1);const state=new RopeWorld().physicsSnapshot();assert.equal(state.floorFriction,PHYSICS_PARAMETERS.floorFriction);assert.equal(state.stepsPerSecond,120);});
...
test('more than 100 sources have distinct 3D anchors and finite physics',()=>{const w=new RopeWorld({wireCount:101,materials:['gold','silver','pink']});assert.equal(w.feeders.length,101);assert.equal(new Set(w.feeders.map(r=>`${r.anchorX},${r.anchorZ}`)).size,101);for(let i=0;i<5;i++)w.step(1/60);for(const r of w.ropes)for(const p of r.nodes)assert.ok(Number.isFinite(p.x+p.y+p.z));});
